...
const CarruselInfinito: React.FC<CarouselProps> = ({
    ...
    return (
        <div className="overflow-hidden relative w-[500px] h-[300px] mx-auto rounded-lg shadow-lg bg-gray-800">
            ...
            <button
                onClick={prev}
                className="absolute left-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full shadow bg-white/80 text-gray-800 hover:bg-white"
            >
                <ChevronLeft size={30} />
            </button>
            <button
                onClick={next}
                className="absolute right-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full shadow bg-white/80 text-gray-800 hover:bg-white"
            >
